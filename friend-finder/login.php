...
//On vérifie que le si les conditions sont respectés//
  $result=$requete->fetch();
    if(!$result){
      ...
      header('Refresh: 5; URL=../login.html'); // rafraîchit la page au bout 4 de secondes //
  }
    else{
      // si le pseudo/mdp correct //
      session_start(); // on démarre la session //
      $_SESSION['id']=$result['id']; // on vérifie que l'id de la base est équivalent à l'id qu'on à reçu //
      $_SESSION['pseudo']=$result['pseudo']; // on vérifie que le pseudo de la base est équivalent à l'id qu'on à reçu //
      echo "Pseudo : ",$_SESSION['pseudo'],"<br> ID : ",$_SESSION['id'];
