...
class entreprise
{
		...
		/* ---------------------- */
			...
			Public function entreprise ( $id_ent, $nom_ent, $nom_resp, $prenom_resp, $mail_ent, $rue_ent, $ville_ent, $cp_ent, $photo_ent, $login_ent, $mdp_ent, $etat_ent, $id_act)
			{
			...
			Public function set_id_act($id_act)
			{
				 $this-> id_activite = $id_act;
			}
			...
			Public function ajout_entreprise ($objet, $conn)
				{
					...
					print $SQL = " INSERT INTO entreprise values (NULL, '$nom_ent', '$nom_resp', '$prenom_resp', '$mail_ent', '$rue_ent', '$ville_ent', '$cp_ent', '$photo_ent', '$login_ent', '$mdp_ent', '0','$id_act')";
				...
				Public function affiche_entreprise ($objet, $conn)
				{
					...
					$mdp_ent = $objet->get_mdp_entreprise();
					$etat_ent = $objet->get_etat_entreprise();
					$SQL = " SELECT *  From entreprise WHERE id_entreprise = '$id_ent'";
					$Req = $conn -> query ($SQL) or die (' Erreur affichage entreprise ');
					$Res = $Req -> fetch ();
					return $Res;
				}

				Public function login_entreprise ($objet, $conn)
				{
					$id_ent = $objet->get_id_entreprise();
					$nom_ent = $objet->get_nom_entreprise();
					$nom_resp = $objet->get_nom_responsable();
					...
					print $SQL = " SELECT * FROM entreprise
			              WHERE login_entreprise = '$login_ent'
			              AND mdp_entreprise = '$mdp_ent'";
					$Req = $conn -> query ($SQL) or die (' Erreur affichage entreprise ');
					$Res = $Req -> fetch ();
					return $Res;
				}
